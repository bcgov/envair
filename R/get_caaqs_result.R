...
get_caaqs_metrics <- function(parameter, years) {
  ...
  aq_data <- importBC_data_avg(
    parameter = parameter_select, years = min_yrs:max_yrs,
  ...
  aq_completeness <- get_data_completeness(
  ...
  aq_valid_ <- ungroup(aq_completeness_summary) %>%
    ...
    mutate(
      valid_quarter = (count >= 60),
      valid_annual = (count >= 75),
      valid_over50 = ifelse(parameter %in% c("NO2", "SO2"), (count >= 50), FALSE)
    ) %>%
    left_join(df_data_requirement,
      by = c("parameter", "data_count_category"),
  ...
  aq_valid_data_ <- aq_valid_ %>%
    left_join(aq_data_wide,
      by = c("parameter", "station_name", "instrument", "year", "metric"),
      relationship = "many-to-many"
    ) %>%
  ...
  df_result <- df %>%
    ...
    group_by(
      parameter, station_name, instrument,
    ...
    mutate(
      metric_value = ifelse(years_averaged == 3, value_3yr, value),
    ...
    ungroup() %>%
    select(
      parameter, station_name, instrument, year, metric,
...
get_management <- function(df) {
  ...
  df_calculate <- df_calculate %>%
    ...
    mutate(
      dev_ll = metric_value_rounded - lower_limit,
